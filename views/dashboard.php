...
<?=(isset($_COOKIE['authCode']) ? ($user->activated != 1 ? '<div class="alertSticky">
    ...
    <div id="container">
        <nav id="sidebar">
            ...
            <a href="/" id="sidebarElement" class="sidebarElementClicked">
...
<div id="SysContentPage">
            <div id="tabs">
                <a id="tab" class="tab_active" href="/dashboard">
                    Подписки
                </a>
                <a id="tab" href="/popular">
                    Популярное
                </a>
                <a id="tab" href="/explore">
                    Обзор
                </a>
            </div>
            ...
            ?>
            <div class="alert__banned">
                <h2>
                    Вам был ограничен доступ к Жабблеру.
                </h2>
                <div>
                    <span>
                        Вы были заблокированы за <?=$user->reason?>.<br>
            ...
            <div id="posts_lent">
            <?php Zhabs::getAllZhabsFollowed($user->userID, 0) ?>
            </div>
            <script>
                <?php if(DB::RowCount("SELECT * FROM zhabs LEFT JOIN users ON userID = zhabBy LEFT JOIN follows ON followTo = zhabBy WHERE followBy = :by", [":by" => $user->userID]) != 0){ ?>
                ...
                function infScroll(){
                    $(document).scroll(function(e){
                        ...
                        if($(window).scrollTop() == $(document).height() - $(window).height()){
                            ...
                            $.post("/API/zhabs/totalCountByFollowed", function (data) {
                                if($("#post").length < Number(data)) {
                                    $(window).off("scroll");
                                    $.post("/API/zhabs/getAllZhabsFollowed", {lastid:Number($("#posts_lent .RealZhab").last().attr("realid"))}, function(data){
                                        $("#posts_lent .loading").remove();
                                        $("#posts_lent").append(data);
                                    })
                                }
                            })
                        }
