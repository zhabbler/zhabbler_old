...
if($profile->entered_all != 1){
    User::redirect("/messages");
}
if(!empty($profile->reason)){
    Alert::PushMessage("Пользователь был заблокирован.");
    User::redirect("/messages");
}
...
$show = false;
if($profile->activated != 1 || $user->activated != 1){
    $show = false;
}else{
...
<?=(isset($_COOKIE['authCode']) ? ($user->activated != 1 ? '<div class="alertSticky">
    ...
    <div id="container">
        <nav id="sidebar">
            ...
            <a href="/messages" id="sidebarElement" class="sidebarElementClicked">
                <div id="sidebarElementIcon">
        ...
        <main id="mainContent">
<div id="SysContentPage" style="min-height:100vh;">
            <div id="MessageAppPerson">
                <a href="#" onclick="location.href = '/messages';return false;" id="MessageAppBackToMessagesLink">
                ...
                <div id="MessageAppPersonInformation">
                    <a href="/profile/<?=$profile->nickname?>">
                        @<?=$profile->nickname?>
                    </a>
                </div>
                <a href="/profile/<?=$profile->nickname?>" id="MessageAppPersonAvatar">
                    <img src="<?=$profile->profileImage?>">
                </a>
            </div>
            <div id="MessageAppMessages" class="id__MessageAppMessages">
                <span class='loading'><span id='ldF'></span><span id='ldS'></span><span id='ldT'></span></span>
            </div>
            <div id="MessageAppWriteMessage">
                <?php
                if($show === true){
                ?>
            ...
            <script>
                $(document).ready(function(){
                    setInterval(() => {
                        $.post("/API/messages/showMessages", {to:<?=$profile->userID?>}, function(data){
                            $('#MessageAppMessages .loading').remove();
                            var messagesBox = $('.id__MessageAppMessages');
                            var currentScrollPos = messagesBox.scrollTop();
                            var elementHeight = messagesBox[0].scrollHeight;
                            var scroll = false;
                            if (messagesBox.height() + currentScrollPos >= elementHeight) {
                                scroll = true;
                            }
                            ...
                            if (scroll == true) {
                                var height = messagesBox[0].scrollHeight;
                                messagesBox.scrollTop(height);
                            }
                        });
                    }, 250);
                    $("#MessageAppWriteMessage button").click(function(){
                        $.post("/API/messages/sendMessage", {to:<?=$profile->userID?>, message:$("#MessageAppWriteMessage #messageContent").val()});
                        $("#MessageAppWriteMessage #messageContent").val("");
                    });
                });
            </script>
            </div>
