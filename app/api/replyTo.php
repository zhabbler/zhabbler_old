...
if(isset($_POST['content']) && isset($_POST['title']) && isset($_POST['inReplyTo'])){
    $inReplyTo = $_POST['inReplyTo'];
    $zhabInfo = Zhabs::getZhabInformation($_POST['inReplyTo']);
    if($zhabInfo->zhabWhoCanRepost == 1){
        Alert::PushMessage("Не удалось отправить ответ на жаб.");
        User::redirect("/");
        die();
    }
    if(DB::RowCount("SELECT * FROM zhabs WHERE zhabURLID = :URLID", [":URLID" => $inReplyTo]) == 0){
        Alert::PushMessage("Не удалось отправить ответ на жаб.");
        User::redirect("/");
        die();
    }
    ...
    if(Text::Null($_POST['content'])){
        ...
        Alert::PushMessage("Зачем тебе название с пробелами?");
    }else{
        ...
        DB::Query("INSERT INTO zhabs(zhabURLID, zhabBy, zhabTitle, zhabContent, zhabUploaded, zhabRepliedTo, zhabWhoCanComment, zhabWhoCanRepost) VALUES(:urlid, :by, :title, :content, :date, :URLID, :wcc, :wcr)", false, false, [
            ":urlid" => $urlID,
            ":by" => $user->userID,
            ":title" => $title,
            ":content" => $content,
